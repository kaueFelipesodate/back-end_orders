...
Projeto dividido em duas aplicações Spring Boot que atendem a prova técnica:

- `cotefacil_api1` - gateway de autenticação JWT e proxy das rotas de pedidos.
...
- `JWT_EXPIRATION`: expiração do token em milissegundos, padrão `3600000` - 1 Hora
...
- `API2_BASE_URL`: URL base da API 2, padrão `http://localhost:8081`
...
- Se você quiser usar outro usuário ou outra senha, ajuste esses campos no [docker-compose.yml] antes de subir os containers.
...
Os mesmos endpoints de pedidos acima estão disponíveis diretamente na API 2 com a validação do token.
...
- No `POST /api/orders` e no `PUT /api/orders/{id}`, se o payload enviar `totalAmount`, a API usa esse valor; se vier `null` ou `0`, a API calcula o total somando os `subtotal` dos itens(reduce).
...
- Exemplo completo de listagem:
  - `GET /api/orders?page=1&size=10&orderBy=createdDate&orderDir=DESC&filters%5Bid%5D=1`
  - `GET /api/orders?page=1&size=10&orderBy=createdDate&orderDir=DESC&filters%5Bstatus%5D=PENDING`
  - `GET /api/orders?page=1&size=10&orderBy=createdDate&orderDir=DESC&filters%5BorderDate%5D=2025-12-02T10:30:00`
